...
const RIGHT_MONITOR_EXPECTED_INSET_REGEX_SOURCE = 'calc\\(29\\.19921875% - 1px\\)\\s+calc\\(25\\.1953125% - 1px\\)\\s+calc\\(17\\.08984375% - 1px\\)\\s+calc\\(24\\.4140625% - 1px\\)';
...
test('right monitor frame is vertically corrected and uses flipped screen bounds', () => {
  ...
  assert.match(
    source,
    new RegExp(`\\.right-monitor-screen-window\\s*\\{[^}]*inset:\\s*${RIGHT_MONITOR_EXPECTED_INSET_REGEX_SOURCE};`, 's'),
    'Expected right monitor screen bounds to align with the vertically flipped right frame hole',
  );
  assert.match(
    source,
    new RegExp(`\\.monitor-group-right\\s*>\\s*\\.monitor-shadow-layer\\s*\\{[^}]*inset:\\s*${RIGHT_MONITOR_EXPECTED_INSET_REGEX_SOURCE};`, 's'),
