# Naimean_v3 – Master Architecture & Implementation Roadmap
...
Several systems rely on local-only storage or hybrid persistence. Users may believe data is saved when it is not.
...
Calendar should become fully server-backed. Do **not** store all events as a single JSON blob. Use relational SQLite tables instead.
...
Before adding Calendar, Preferences, and User profiles, refactor handlers into modular functions:
...
Long-term direction — separate Durable Objects per domain:
...
Benefits: Profiles, Achievements, Inventory, Progression, Personalization
...
Benefits: Cross-device consistency, Accessibility support, Future customization
...
Benefits: Faster navigation, Better discoverability, Reduced user confusion
...
Benefits: Better retention, Reduced confusion, Improved first impressions
...
Important due to CRT effects, animated scenes, and media-heavy content.
