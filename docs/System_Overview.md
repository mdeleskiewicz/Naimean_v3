...
| Object Storage | R2 (`ASSETS_STORAGE`) | Asset/object storage integration point |
...
- CI workflow currently invokes `wrangler` with `--config wrangler.jsonc` in `.github/workflows/deploy.yml`; keep config paths aligned when updating deployment automation.
