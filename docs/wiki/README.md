...
Cloudflare's CLI and the `wrangler.toml` config file. Covers every config section (name, main, assets, vars, durable_objects, migrations, observability), the difference between `vars` and secrets, and common Wrangler commands.
...
| **D1 (diagnostic probe)** | `env.DB` | `/api/db-test` in `src/worker.js` |
| **Observability** | `[observability] enabled = true` | Automatic — no code needed |
| **Wrangler** | `wrangler.toml` | CLI deploy tool |
